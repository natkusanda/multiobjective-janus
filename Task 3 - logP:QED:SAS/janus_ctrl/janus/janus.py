...
from .janus_norm import normalize_and_sum, min_dist_utopia
...
class JANUS():
    ...
    def __init__(self, work_dir: str, fitness_function: Callable,
        ...
        idx, __ = normalize_and_sum(np.array(init_fitness))
    ...
    def run(self):
        ...
        for gen_ in range(self.generations):
            ...
            ### ---- NAT: START NORM - Lines 177 - 183 ----
            idx_temp, norm_vals = normalize_and_sum(np.array(self.fitness))
            ...
            print("exploration mutation before filter :", len(mut_smi_explr))
            #og mutation for getting all generated

            mut_smi_explr = self.check_filters(mut_smi_explr)
            print("exploration mutation after filter :", len(mut_smi_explr))
            ...
            if gen_ == 0:
                ...
                if self.use_NN_classifier == True:
                    ...
                    ### ---- NAT: START NORM - Lines 240 - 245 ----
                    for item in self.smiles_collector: 
                        train_smiles.append(item)
                        fit_val.append(self.smiles_collector[item][0])

                    __, norm_vals = normalize_and_sum(np.array(fit_val))
                    for i in range(len(train_smiles)): 
                        pro_val.append(norm_vals[i])
                    train_and_save_classifier(train_smiles, pro_val, generation_index=nn_tag)
                    ### ---- NAT: END NORM ----
            ...
            idx_sort, norm_vals = normalize_and_sum(np.array(self.fitness))
            ### ---- NAT: END NORM ----
            ...
            norm_sort = np.array(norm_vals)[idx_sort]
            ...
            if self.verbose_out:
                ...
                with open("./RESULTS/" + str(gen_) + "_DATA/norm_explore.txt", "w") as f:
                    f.writelines(["{} ".format(x) for x in norm_sort])
                    f.writelines(["\n"])
            else:
                with open("./RESULTS/fitness_explore.txt", "w") as f:
                    ...
                    f.writelines(["\n"])
                # ---- NAT: MULTIOPT PRINTOUT
                with open("./RESULTS/norm_explore.txt", "w") as f:
                    f.writelines(["{} ".format(x) for x in norm_sort])
                    f.writelines(["\n"])
            ...
            '''
            ### ---- NAT: START NORM Line 416 ----
            idx_sort, norm_vals = normalize_and_sum(np.array(self.fitness_loc))
            ### ---- NAT: END MULTIOPT ----
            ...
            if self.verbose_out:
                with open(
                    ...
                    f.writelines(["\n"])
                # ---- NAT: MULTIOPT PRINTOUT
                with open("./RESULTS/" + str(gen_) + "_DATA/norm_local_search.txt", "w") as f:
                    f.writelines(["{} ".format(x) for x in norm_sort])
                    f.writelines(["\n"])
            else:
                with open("./RESULTS/fitness_local_search.txt", "w") as f:
                    f.writelines(["{} ".format(x) for x in fitness_sort])
                    f.writelines(["\n"])
                with open("./RESULTS/norm_local_search.txt", "w") as f:
                    f.writelines(["{} ".format(x) for x in norm_sort])
                    f.writelines(["\n"])
            ...
            '''
            ### ---- NAT: START NORM - Lines 469 - 482 ----

            idx_sort, __ = normalize_and_sum(np.array(self.fitness))
            ...
            if gen_ == self.generations - 1:
                ...
                idx_sort, scalarizer_vals = normalize_and_sum(np.array(all_fit))
                ...
                for j in range(len(smiles_sort)):
                    csv_vals.append((j,smiles_sort[j],fitness_sort[j][0],fitness_sort[j][1],fitness_sort[j][2]))
                df = pd.DataFrame(csv_vals, columns =['i','smi','qed','logp','sas'])
