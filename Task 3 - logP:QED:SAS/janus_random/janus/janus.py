...
from .janus_olympus import create_value_space, create_scalarizer, scalarize_and_sort, randomize_and_sort, min_dist_utopia
...
class JANUS():
    ...
    def __init__(self, work_dir: str, fitness_function: Callable,
        ...
        idx, __ = randomize_and_sort(np.array(init_fitness))
    ...
    def run(self):
        ...
        for gen_ in range(self.generations):
            ...
            idx_temp, scalarizer_vals = randomize_and_sort(np.array(self.fitness))
            print("Step 1 first sort: ", np.array(self.population)[idx_temp], np.array(self.fitness)[idx_temp])

            keep_smiles, replace_smiles = self.get_good_bad_smiles(
                scalarizer_vals, # 0-1 is best-worst
                self.population,
                self.generation_size
            )
            ...
            if gen_ == 0:
                ...
                if self.use_NN_classifier == True:
                    ...
                    __, scalarizer_vals = randomize_and_sort(np.array(fit_val))
            ...
            idx_sort, scalarizer_vals = randomize_and_sort(np.array(self.fitness))
            ### ---- NAT: END MULTIOPT ----
            ...
            idx_sort, scalarizer_vals = randomize_and_sort(np.array(self.fitness_loc))
            ### ---- NAT: END MULTIOPT ----
            ...
            idx_sort, __ = randomize_and_sort(np.array(self.fitness))
            ...
            if gen_ == self.generations - 1:
                # Book keeping: compute actual fitnesses, record in CSV
                ...
                #fin_scalarizer = create_scalarizer(properties, objectives, kind, supplement)
    ...
    def get_good_bad_smiles(fitness, population, generation_size):
        ...
        this function splits the population into two lists: keep_smiles & replace_smiles. 
