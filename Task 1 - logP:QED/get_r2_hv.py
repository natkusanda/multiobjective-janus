...
def compute_pareto(optim_type, w_ref, mode):
    if optim_type == 'zinc':
        ...
        df = pd.read_csv("RESULTS/smiles_collector_" + optim_type + ".csv")
        smiles = list(df['smi'])
        logp = list(df['logp'])
        qed = list(df['qed'])
    ...
    new_ref = np.array([-w_ref[0], -w_ref[1]])
    if mode == 'r2':
    ...
    if mode == 'r2':
        ...
        for optim_type in optim_types:
            ...
            if optim_type == 'zinc':
                ...
                abs_ref = [max(abs_ref[0],find_ref[0]),max(abs_ref[1],find_ref[1])]
