...
sas_front = [sas[i] for i in range(len(idx)) if pareto[i] == 1]
docking_front = [docking[i] for i in range(len(idx)) if pareto[i] == 1]
pareto_front = np.column_stack((sas_front,docking_front))
# pareto_front[:,0] = - pareto_front[:,0]
# pareto_front[:,1] = - pareto_front[:,1]
# logp_worst = -6.8762
# qed_worst = 0.025882
# w_ref = np.array([-logp_worst,-qed_worst])
sas_worst = np.max(sas)
docking_worst = np.max(docking)
w_ref = np.array([sas_worst, docking_worst])
...
kind = 'Hypervolume'
supplement = [0,0]
data = np.column_stack((sas,docking))
...
with open("zinc_250k_sliced.txt", "w") as f:
    f.writelines(["{}\n".format(x) for x in hv_keep_smiles])
