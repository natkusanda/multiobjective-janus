...
# %%
def normalize_and_sum(fitness_list, task='qed-logp'):
    # %%
    if task == 'qed-logp':
        # first column is QED, second is logp
        norm_vals = 16.66 * fitness_list[:,0] + fitness_list[:,1]
    elif task == 'SAS-1oyt':
        # first column is SAS, second is docking for 1oyt
        # we want to minimize both objectives
        norm_vals = - 20.0 * fitness_list[:,0] - fitness_list[:,1]
    else:
        raise NotImplementedError('Not implemeneted for this task')
